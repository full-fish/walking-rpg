...
test('★ 골드는 어디로 가나 — 유지비가 수입의 25~35%여야 한다 (§4.5)', () => {
  ...
  // T19에 걸음 목표·출석·도감 드랍이 들어와 22% → 약 10%로 내려갔다 — 매일 걷고 켜는 사람은 물약·여관
  // 걱정이 준다를 받아들였다(검수 주석). 물약·여관이 아예 안 쓰이는 쪽으로 무너지는 것만 막는다
  expect(invest.upkeep, '유지비 비중 (투자형)').toBeGreaterThan(0.08);
});
